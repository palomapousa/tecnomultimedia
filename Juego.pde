class Juego {
  ...
  Juego () {
    ...
    Esposo = new Enemigo (random(0, 800), random(0, 500), 30);
    botones = new Botones ();
    gif = new PImage [16];
    for (int f = 0; f<16; f++) {
  ...
  void dibujarJuego() {
    if (jugando) {
      if (victoria()) { 
        ...
        botones.dibujarBoton("Volver a Jugar", color(255, 0, 0), 18, width-200, height-69, width-125, height-38);
        if (frameCount%2 ==1) {
          contador++;
        }
        if (mousePressed && botones.mouseOver (width-200, height-69)) {
          jugando = false;
          Esposo.vida = 8;
        } 
        pop();
      ...
      dibujarInicio();
      inicio(120, height-69);
    }
    Esposo.tiempo();
  }

  void inicio(int ix, int iy) {
    if (mousePressed && botones.mouseOver (ix, iy)) {
      jugando = true;
      Esposo.px=(random(0, 800));
      Esposo.py=(random(0, 500));
      Player.x= width/2;
      Player.y= height/2;
  ...
  void dibujarInicio () {
    ...
    botones.dibujarBoton("Jugar", color(255, 0, 0), 18, 120, height-69, 174, height-38);
  }
